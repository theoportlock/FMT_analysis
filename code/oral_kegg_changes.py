...
data_samples = pd.read_csv("../../data/oralkegg.csv", index_col=0)
samples_data=data_samples.T
#var = 'AMR Gene Family'
#samples_data = data_samples[[*samples_metadata.loc[samples_metadata['Type'] == 'FMT'].index, var]].groupby(var).sum().T
...
significantMatrix = stats.loc[:, (stats < 0.005).any(axis=0)]
