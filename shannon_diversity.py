...
samples_gutsum = pd.DataFrame(samples_gutmsp.agg(shannon, axis=1), columns=['Species richness']).join(samples_metadata)
...
samples_oralsum = pd.DataFrame(samples_oralmsp.agg(shannon, axis=1), columns=['Species richness']).join(samples_metadata)
...
plt.tight_layout()
plt.show()
#plt.savefig("results/shannon_diversity.pdf")
