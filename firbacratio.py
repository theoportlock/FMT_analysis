...
# plot
fig, (donor_ax, gut_ax, oral_ax) = plt.subplots(1, 3, sharey=True, gridspec_kw={'width_ratios': [1, 8, 8]})
...
tmp[0] = 'Donor'
dtmpplot = tmp.reset_index()
sns.boxplot(data = dtmpplot, x=0,y='F/B',ax=donor_ax)
sns.swarmplot(data = dtmpplot, x=0,y='F/B',ax=donor_ax)

#gut
tmp1 = gutfmtatt1.reset_index().groupby('phylum').sum()
tmp=pd.DataFrame()
tmp['F/B'] = tmp1.loc['Firmicutes',:].div(tmp1.loc['Bacteroidetes',:])
gtmpplot = tmp.reset_index()
sns.boxplot(data = gtmpplot, x='Day',y='F/B', hue='Type', ax=gut_ax)
sns.swarmplot(data = gtmpplot, x='Day',y='F/B', hue='Type', ax=gut_ax)

#oral
tmp1 = oralfmtatt1.reset_index().groupby('phylum').sum()
tmp['F/B'] = tmp1.loc['Firmicutes',:].div(tmp1.loc['Bacteroidetes',:])
otmpplot = tmp.reset_index()
sns.boxplot(data = otmpplot, x='Day',y='F/B',hue='Type', ax=oral_ax)
sns.swarmplot(data = otmpplot, x='Day',y='F/B',hue='Type', ax=oral_ax)

sns.despine(trim=True, left=True)
#box_pairs=[(('0','FMT'),('7','FMT')),(('0','FMT'),('30','FMT')),(('0','FMT'),('90','FMT'))]
box_pairs=[(('30','PLACEBO'),('30','FMT'))]
