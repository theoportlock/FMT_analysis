...
sns.heatmap(datt3,cmap='binary',ax=gut_donor_ax, cbar=False, yticklabels=True)
...
# gut fmt
fatt1 = sumofsm.drop('Donor',level=0,axis='columns')
...
sns.heatmap(fatt3,cmap='binary',ax=gut_patient_fmt_ax,cbar=False, yticklabels=True)
#att3.T.plot(kind='bar',stacked=True, ax=gut_patient_fmt_ax, legend=False)
gut_patient_fmt_ax.title.set_text('FMT - Stool')

# gut placebo
patt1 = sumofsm.drop('Donor',level=0,axis='columns')
...
sns.heatmap(patt3,cmap='binary',ax=gut_patient_placebo_ax, yticklabels=True)
#att3.T.plot(kind='bar',stacked=True, ax=gut_patient_placebo_ax)
gut_patient_placebo_ax.title.set_text('Placebo - Stool')

gut_patient_placebo_ax.set_ylabel('')
gut_patient_fmt_ax.set_ylabel('')
