...
#samples_metadata = samples_metadata.loc[samples_metadata.Type != 'DONOR']
...
samples_taxo = gtaxonomy_samples.T
df = samples_taxo.join(samples_metadata[var], how='inner')
...
labscaledDf = pd.DataFrame(scaledDf, index=df.drop(var, axis=1).index,columns=df.drop(var, axis=1).columns)
#pca = PCA()
#tmp = pca.fit(scaledDf, df[var])
lda = PCA().fit(scaledDf, df[var])
results = lda.transform(scaledDf)
coef = pd.DataFrame(lda.coef_,index=lda.classes_,columns=samples_taxo.columns)
sns.heatmap(coef.T[(coef > 0.9).any()],yticklabels=True)
...
df2 = samples_metadata.join(df[['Cluster','PC1','PC2']], how='inner')
fig, ax = plt.subplots()
...
sns.scatterplot(data = df2, x='PC1', y='PC2', style='Type', hue='Days after treatment', palette='coolwarm')
