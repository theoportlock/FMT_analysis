...
cazyme = pd.read_table(file_cazyme, sep = '\t')
...
replacedbycazyme = replacedbygenename.merge(cazyme, how='inner', left_on='gene_name', right_on='gene_name')
del replacedbygenename

fmtcols = [col for col in replacedbycazyme.columns.values if 'FMT' in col]
fmtcols.append('CAZyme')
sumofsm = replacedbycazyme[fmtcols].groupby('CAZyme').sum().compute()

del replacedbycazyme
del gene_info
del cazyme
