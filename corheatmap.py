...
fgut_msp_data = fgut_msp_data.loc[(fgut_msp_data.sum(axis=1) != 0), (fgut_msp_data.sum(axis=0) != 0)]
...
fcorrelations = correlations.loc[
    fgut_msp_data.select_dtypes(include=np.number).columns[1:],
    fffmerged_metadata.select_dtypes(include=np.number).columns]
funcorrected_p_values = uncorrected_p_values.loc[
    fgut_msp_data.select_dtypes(include=np.number).columns[1:],
    fffmerged_metadata.select_dtypes(include=np.number).columns]
#funcorrected_p_values = uncorrected_p_values.loc[fffmerged_metadata.select_dtypes(include=np.number).columns,fgut_msp_data.select_dtypes(include=np.number).columns[4:]].T
shape = funcorrected_p_values.values.shape
significant_matrix = multipletests(funcorrected_p_values.values.flatten())[0].reshape(shape)

#theoeditstart
fsignificant_matrix = pd.DataFrame(significant_matrix, columns=fcorrelations.columns, index=fcorrelations.index)
...
significant_matrix = fsignificant_matrix.to_numpy()
#theoeditend
...
g = sns.clustermap(fcorrelations, cmap="vlag", vmin=-1, vmax=1)
